...
import { getPacienteDetails, updatePaciente, deletePaciente, createConsulta, deleteConsulta, gerarPlanoAlimentarIA, gerarPlanoAlimentarManual, salvarPlanoAlimentar } from '../lib/api';
import { Paciente, Consulta, PlanoAlimentar, PlanoSemanalData, Refeicoes } from '../types';
import {
  LineChart,
  Line,
  XAxis,
...
export const PacientePerfil: React.FC = () => {
  ...
  // AI Meal Plan Generation & Editor State (Prompt 6)
  const [isGeneratingAI, setIsGeneratingAI] = useState(false);
  const [loadingMessage, setLoadingMessage] = useState('Buscando dados do paciente...');
  const [activePlanoData, setActivePlanoData] = useState<PlanoSemanalData | null>(null);
  const [activeDayIndex, setActiveDayIndex] = useState<number>(0);
  const [aiErrorModal, setAiErrorModal] = useState<{ show: boolean; message: string }>({ show: false, message: '' });
  ...
  // Save Plano Alimentar Mutation (Prompt 6)
  const savePlanoMutation = useMutation({
    mutationFn: async () => {
      if (!id || !activePlanoData) throw new Error('Dados do plano inválidos.');
      return await salvarPlanoAlimentar(id, activePlanoData, sessionToken || undefined);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pacienteDetails', id, sessionToken] });
      setActivePlanoData(null);
      setFeedbackMessage({ type: 'success', text: 'Plano alimentar salvo com sucesso!' });
      setTimeout(() => setFeedbackMessage(null), 4000);
    },
    onError: (err: any) => {
      console.error('Erro ao salvar plano alimentar:', err);
      setFeedbackMessage({ type: 'error', text: 'Não foi possível salvar o plano alimentar. Tente novamente.' });
      setTimeout(() => setFeedbackMessage(null), 4000);
    },
  });

  // AI & Manual Generation Handlers (Prompt 6)
  const handleGeneratePlanoAI = async () => {
    if (!paciente) return;
    ...
    const messages = [
      ...
      'IA calculando cardápio nutricional...',
      'Montando sugestões de refeições brasileiras...',
      'Finalizando prescrição semanal...',
    ];
    let msgIdx = 0;
    ...
    try {
      const res = await gerarPlanoAlimentarIA(paciente);
      clearInterval(interval);
      setIsGeneratingAI(false);

      if (res.success && res.data && Array.isArray(res.data.plano_semanal)) {
        setActivePlanoData(res.data);
        setActiveDayIndex(0);
      } else {
        setAiErrorModal({
          show: true,
          message: res.error || 'Não foi possível gerar o plano com IA no momento. Deseja tentar novamente ou criar um Plano Manual?',
        });
      }
    } catch (err: any) {
      clearInterval(interval);
  ...
  const handleGeneratePlanoManual = () => {
    if (!paciente) return;
    const manualData = gerarPlanoAlimentarManual(paciente);
    setActivePlanoData(manualData);
    setActiveDayIndex(0);
    setAiErrorModal({ show: false, message: '' });
  };

  const handleOptionChange = (dayIdx: number, mealKey: keyof Refeicoes, optIdx: number, value: string) => {
    if (!activePlanoData) return;
    const newPlano = JSON.parse(JSON.stringify(activePlanoData)) as PlanoSemanalData;
    if (newPlano.plano_semanal[dayIdx]?.refeicoes[mealKey]) {
      newPlano.plano_semanal[dayIdx].refeicoes[mealKey][optIdx] = value;
      setActivePlanoData(newPlano);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      {/* ========================================================================= */}
      {/* SEÇÃO 3: PLANOS ALIMENTARES (Prompt 6 - IA + Editor Interativo) */}
      {/* ========================================================================= */}
      {mainSection === 'planos' && (
        <div className="space-y-6 animate-fadeIn">
          {/* Header Action Bar */}
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Histórico e gerador de dietas nutricionais automatizadas via IA</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={handleGeneratePlanoAI}
                className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold text-xs hover:from-emerald-700 hover:to-teal-700 shadow-md shadow-emerald-600/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed self-start sm:self-auto"
              >
                {isGeneratingAI ? (
                  <>
              ...
              {!activePlanoData && (
                <button
                  type="button"
                  onClick={handleGeneratePlanoManual}
                  className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 font-semibold text-xs hover:bg-slate-100 transition-colors"
                >
                  <Plus className="w-3.5 h-3.5" />
          ...
          {isGeneratingAI && (
            <div className="bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 border border-emerald-200/80 rounded-2xl p-8 text-center space-y-4 shadow-sm animate-pulse">
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-800">Criando Prescrição Alimentar Personalizada</h4>
                <p className="text-xs text-emerald-700 font-semibold mt-1">{loadingMessage}</p>
              </div>
            </div>
          )}

          {/* Editor Mode: Active Plan Editing (Prompt 6 Requirement 4 & 5) */}
          {activePlanoData && !isGeneratingAI && (
            <div className="bg-white rounded-2xl border border-emerald-300 shadow-lg p-6 space-y-6 animate-scaleUp">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      <h4 className="text-base font-bold text-slate-900">Cardápio Semanal em Edição</h4>
                      <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800">
                        Editável
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">
                      Ajuste as refeições e opções abaixo antes de confirmar o salvamento.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={() => setActivePlanoData(null)}
                  ...
                  <button
                    ...
                    onClick={() => savePlanoMutation.mutate()}
                    className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 shadow-md shadow-emerald-600/20 disabled:opacity-60 transition-all"
                  >
                    {savePlanoMutation.isPending ? (
                      <>
              ...
              <div className="flex items-center space-x-1.5 overflow-x-auto pb-2 border-b border-slate-100 scrollbar-none">
                {activePlanoData.plano_semanal.map((diaObj, idx) => (
                  <button
                    key={diaObj.dia || idx}
                    type="button"
              ...
              {/* Active Day Meals Form */}
              {activePlanoData.plano_semanal[activeDayIndex] && (
                <div className="space-y-6 pt-2">
                  <div className="flex items-center justify-between">
                    <h5 className="text-sm font-bold text-slate-800 flex items-center space-x-2">
                      <Calendar className="w-4 h-4 text-emerald-600" />
                      <span>Refeições para {activePlanoData.plano_semanal[activeDayIndex].dia}</span>
                    </h5>
                    <span className="text-xs text-slate-400">5 opções por refeição</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(
                      [
                        { key: 'cafe_da_manha', title: '☕ Café da Manhã', badgeColor: 'bg-amber-50 text-amber-800 border-amber-200' },
                        { key: 'lanche_manha', title: '🍎 Lanche da Manhã', badgeColor: 'bg-emerald-50 text-emerald-800 border-emerald-200' },
                        { key: 'almoco', title: '🥗 Almoço', badgeColor: 'bg-teal-50 text-teal-800 border-teal-200' },
                        { key: 'lanche_tarde', title: '🥪 Lanche da Tarde', badgeColor: 'bg-sky-50 text-sky-800 border-sky-200' },
                        { key: 'jantar', title: '🍲 Jantar', badgeColor: 'bg-indigo-50 text-indigo-800 border-indigo-200' },
                      ] as const
                    ).map((mealGroup) => {
                      const mealKey = mealGroup.key as keyof Refeicoes;
                      const optionsList =
                        activePlanoData.plano_semanal[activeDayIndex].refeicoes[mealKey] || [];

                      return (
                        <div
                          key={mealGroup.key}
                          className="bg-slate-50/70 p-4.5 rounded-2xl border border-slate-200/80 space-y-3"
                        >
                          <div className="flex items-center justify-between border-b border-slate-200/60 pb-2">
                            <h6 className="text-xs font-bold text-slate-800">{mealGroup.title}</h6>
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${mealGroup.badgeColor}`}
                            >
                              {optionsList.length} opções
                            </span>
                          </div>
                          ...
                          <div className="space-y-2">
                            {optionsList.map((optText, optIdx) => (
                              <div key={optIdx} className="flex items-center space-x-2">
                                <span className="text-[11px] font-bold text-slate-400 w-4 text-right">
                                  {optIdx + 1}.
                                </span>
                                <input
                                  type="text"
                                  value={optText}
                                  onChange={(e) =>
                                    handleOptionChange(activeDayIndex, mealKey, optIdx, e.target.value)
                                  }
                                  className="flex-1 px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800"
                                />
                              </div>
                            ))}
                          </div>
          ...
          {/* History Mode (Prompt 6 Requirement 1 & 6) */}
          {!activePlanoData && (
            <div>
              {planos.length === 0 ? (
                <div className="py-16 text-center bg-white rounded-2xl border border-slate-200/80 p-8 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                      Gere um plano automatizado com IA considerando o perfil, metas e restrições de {paciente.nome}.
                    </p>
                  </div>
                  <button
                    type="button"
                    disabled={isGeneratingAI}
                    onClick={handleGeneratePlanoAI}
                    className="inline-flex items-center space-x-1.5 px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 transition-colors shadow-sm"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {planos.map((plano) => {
                    const isStructured =
                      typeof plano.conteudo === 'object' &&
                      plano.conteudo !== null &&
                      'plano_semanal' in plano.conteudo;
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-100 flex items-center space-x-1">
                              <Sparkles className="w-3 h-3 text-emerald-600" />
                              <span>{isStructured ? 'Plano Semanal' : 'Prescrição'}</span>
                            </span>
                          ...
                          <h4 className="text-sm font-bold text-slate-800 group-hover:text-emerald-700 transition-colors">
                            Plano Alimentar de {new Date(plano.created_at).toLocaleDateString('pt-BR')}
                          </h4>

                          <p className="text-xs text-slate-500 line-clamp-2">
                            {isStructured
                              ? `Cardápio completo de 7 dias (${plano.conteudo.plano_semanal.length} dias cadastrados)`
      ...
      {/* Modal: Visualizar Conteúdo do Plano Alimentar Histórico */}
      {selectedPlano && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-3xl w-full shadow-2xl border border-slate-200 overflow-hidden animate-scaleUp">
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto space-y-4">
              {typeof selectedPlano.conteudo === 'object' &&
              selectedPlano.conteudo !== null &&
              'plano_semanal' in selectedPlano.conteudo ? (
                <div className="space-y-6">
                  {(selectedPlano.conteudo as PlanoSemanalData).plano_semanal.map((diaItem, idx) => (
                    <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                      <h4 className="text-xs font-bold text-emerald-800 uppercase tracking-wider border-b border-slate-200 pb-1.5">
                        {diaItem.dia}
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        {Object.entries(diaItem.refeicoes).map(([mKey, options]) => (
                          <div key={mKey} className="bg-white p-3 rounded-lg border border-slate-100">
                            <span className="font-bold text-slate-700 capitalize block mb-1">
                              {mKey.replace('_', ' ')}:
                            </span>
                            <ul className="list-disc list-inside text-slate-600 space-y-0.5 text-[11px]">
                              {Array.isArray(options) &&
                                options.map((opt: string, oIdx: number) => <li key={oIdx}>{opt}</li>)}
                            </ul>
                          </div>
                        ))}
                      </div>
      ...
      {/* Modal / Toast: Erro da IA (Prompt 6 Requirement: Fallback Toast/Modal) */}
      {aiErrorModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 p-6 space-y-4 animate-scaleUp text-center">
            <div className="w-12 h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center mx-auto">
              <AlertCircle className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900">Atenção na Geração</h3>
              <p className="text-xs text-slate-600 mt-2 leading-relaxed">{aiErrorModal.message}</p>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button
                type="button"
                onClick={handleGeneratePlanoAI}
                className="w-full py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 shadow-sm flex items-center justify-center space-x-1.5"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                <span>Tentar Novamente com IA</span>
              </button>
              ...
              <button
                type="button"
                onClick={() => setAiErrorModal({ show: false, message: '' })}
